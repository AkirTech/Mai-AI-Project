...
class MainApplication(ttk.Window):
    def __init__(self,geometry,title,lang):
        ...
        self.lang_data:dict = load_lang(lang)["UI"]
        ...
        self.auto_play_check = ttk.Checkbutton(self.left_column, text=self.lang_data["app_auto_play_label"],
                                               variable=ttk.BooleanVar(value=True))
        self.auto_play_check.pack(padx=5, pady=10,fill="x",anchor=ttk.N)
        ...
        self.out_label = ttk.Label(self.right_column, text=self.lang_data["app_answer_list_label"],width=60)
    ...
    def on_button_click(self):
        ...
        is_sget_ok = sget_audio(content=self.ai_answer,
                   lang="en",top_k=self.top_k,
                   temperature=1,
                   ref_audio_path="res\\ref_audio.wav",
                   prompt_text=self.user_prompt,
                   prompt_lang="en")
        if is_sget_ok == 0 and self.auto_play_check.instate(["selected"]):
